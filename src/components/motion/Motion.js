...
export default {
    ...
    mounted() {
        ...
        const ideal = this.defineInitialValues(
            this.currentValues ,
    ...
    methods: {
            ...
            this.animationId = raf( () => {
                ...
                const currentFrameCompletion =
                    ( this.accumulatedTime -
                        Math.floor( this.accumulatedTime / msPerFrame ) *
                            msPerFrame ) /
                    msPerFrame
                const framesToCatchUp = Math.floor(
        ...
        animateValues( {
            ...
            for ( const key in realValues ) {
                // istanbul ignore if
                if ( !Object.prototype.hasOwnProperty.call( realValues , key ) )
                    continue
                ...
                if ( isArray( realValues[ key ] ) || isObject( realValues[ key ] ) ) {
                    ...
                    if ( !idealValues[ key ] ) {
                        ...
                        const current = this.defineInitialValues(
                            this.realValues[ key ] ,
                            null ,
                        )
                ...
                currentVelocities[ key ] =
                    newIdealVelocity +
                    ( nextIdealVelocity - newIdealVelocity ) *
                        currentFrameCompletion
