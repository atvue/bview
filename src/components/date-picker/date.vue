        ...
        >
            <div :class="`${b}-date-ctrl-box-nei`">
                <span
                    v-if="type == 'time'"
                    :class="
                        `${b}-date-select-time-btn` +
                            (cDate.day ? '' : ` ${b}-date-st-disabled`)
                    "
                    @click="$_switchPanel"
                >
    ...
    methods: {
        ...
        $_setDate() {
            ...
            for ( var i = 0 ;i < 42 ;i++ ) {
                ...
                //上次的选择
                if (
                    time.year == this.selectDate.year &&
                    time.month == this.selectDate.month &&
                    time.day == this.selectDate.day
                ) {
        ...
        $_selectDay( date ) {
            if ( date.disabled || this.day == date.day ) {
                return
            }
            if ( this.preSelectDay ) {
                this.preSelectDay.class = this.preSelectDay.class.replace(
                    / *cday/ ,
                    `` ,
                )
            }
